...
f = @(t,x) [r*x(1)*(1 - x(1)) - h*x(1)* x(2); 
            gamma*x(1)* x(2) + (m2*x(2)*x(3))/(v+x(2)) - d*x(2);
            x(3)*(1 - x(3)) + (m3*x(2)*x(3))/(v+x(2)) + m4*x(1)*x(3)];
...
figure(1);
set(gcf, 'Color', 'w');
plot3(xa(:,1),xa(:,2),xa(:,3),'v','Linewidth',3); hold on;
...
figure(2)
plot(t, xa(: ,1),'r.', t , xa(: ,3),'v', t , xa(: ,2),'b.')
% title('Time');
xlabel('Time t');
ylabel('Solution y');
